...
async function parseMultipart(req: NextApiRequest): Promise<{ name: string; access: string; fileBuffer: Buffer; filename: string; mimetype: string } | null> {
    ...
    req.on("end", () => {
      try {
        ...
        for (const part of parts) {
          ...
          if (part.includes('name="access"')) {
            const match = part.split("\r\n\r\n");
            if (match[1]) access = match[1].replace(/\r\n--$/, "").trim();
          }
        ...
        resolve({ name, access, fileBuffer, filename, mimetype });
      } catch (e) {
        reject(e);
      }
    });
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const { name, access, fileBuffer, filename, mimetype } = parsed;
    ...
    let index: { id: string; name: string; url: string; filename: string; access: string; uploadedAt: string; uploadedBy: string }[] = [];
    try {
      const { blobs } = await list({ prefix: "republic/index.json" });
      if (blobs && blobs.length > 0) {
        const indexRes = await fetch(blobs[0].url + "?t=" + Date.now());
