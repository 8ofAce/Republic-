  ...
  const handleAddDev = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/managedev", {
        method: "POST",
        headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
        body: JSON.stringify({ name: devName.trim(), title: devTitle.trim(), image: devImage.trim() }),
      });
      const data = await res.json();
      if (!res.ok) { setDevError(data.error || "Failed to add dev"); }
      else { setDevSuccess(true); setDevName(""); setDevTitle(""); setDevImage(""); fetchDevs(); setTimeout(() => setDevSuccess(false), 3000); }
  ...
  return (
    <>
                  ...
                  {devs.map((dev) => (
                    <div key={dev.id} className="dev-card">
                      ...
                      <div className="dev-info">
                        ...
                        <span className="dev-title">{dev.title}</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="tab-content">
              ...
              <div className="upload-box">
                <form onSubmit={handleAddDev} className="upload-form">
                  ...
                  <div className="field">
                    <label>Profile Image URL <span className="label-optional">(optional)</span></label>
                    <input type="url" value={devImage} onChange={(e) => setDevImage(e.target.value)} placeholder="https://example.com/photo.jpg" />
                  </div>
              ...
              {files.length === 0 ? <p className="muted">No files yet.</p> : (
                <div className="manage-list" style={{ marginBottom: "40px" }}>
                  {files.map((file) => (
      ...
      <style jsx>{`
        ...
        .dev-avatar { width: 72px; height: 72px; border-radius: 50%; overflow: hidden; background: var(--surface2); border: 2px solid var(--border); display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
        .dev-img { width: 100%; height: 100%; object-fit: cover; }
        .dev-initial { font-family: 'Bebas Neue', sans-serif; font-size: 32px; color: var(--accent); }
        .dev-info { display: flex; flex-direction: column; gap: 4px; }
        .dev-name { font-size: 15px; font-weight: 500; color: var(--text); }
        .dev-title { font-size: 12px; color: var(--muted); letter-spacing: 0.5px; text-transform: uppercase; }
