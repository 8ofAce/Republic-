...
export default function Home() {
  ...
  const getRole = () => localStorage.getItem("republic_role") || "";
  ...
  const fetchFiles = async (t: string) => {
    ...
    try {
      ...
      if (data.locked) {
        ...
        setFiles(Array.isArray(data.files) ? data.files.reverse() : []);
      }
    } catch {
      setFiles([]);
    }
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    if (uploadType === "link") {
      try {
        ...
        else { setUploadSuccess(true); setUploadName(""); setUploadLink(""); fetchFiles(t); setTimeout(() => setUploadSuccess(false), 3000); }
    ...
    try {
      ...
      if (!res.ok) { setUploadError(data.error || "Upload failed"); }
        ...
        fetchFiles(t); setTimeout(() => setUploadSuccess(false), 3000);
      }
  ...
  const handleAddUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) { setUserError(data.error || "Failed"); }
        ...
        fetchUsers(t); setTimeout(() => setUserSuccess(false), 3000);
      }
          ...
          {tab === "files" && (
            <div className="tab-content">
              <div className="tab-header">
                ...
                {currentRole && <p className="tab-subtitle">Signed in as <strong>{currentUser}</strong> · <span style={{ color: currentRole === "premium" ? "#c8a96e" : currentRole === "admin" ? "#2ecc71" : "#7fb3d3", textTransform: "capitalize" }}>{currentRole}</span></p>}
                {!currentRole && <p className="tab-subtitle">Login to access files</p>}
              </div>
              {filesLocked ? (
                ...
                <div className="manage-list">
                  {users.map((user) => (
                    <div key={user.id} className="manage-row">
                      ...
                      <div className="manage-actions">
                        <button className="action-delete" onClick={() => handleDeleteUser(user)}>Remove</button>
                      </div>
                    </div>
                  ))}
                </div>
